...
def train(args, train_dataset, model, tokenizer):
    ...
    save_count = 0
    save_point = [int(t_total*0.05), int(t_total*0.1), int(t_total*0.2), int(t_total*0.5)]
    print(save_point)
    out_list = [5,10,20,50]
    global_step = 0
    epochs_trained = 0
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step in save_point:
                    logs = {}
                    if (
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step in save_point:
                    # Save model checkpoint
                    output_dir = os.path.join(args.output_dir, "checkpoint-{}".format(out_list[save_count]))
