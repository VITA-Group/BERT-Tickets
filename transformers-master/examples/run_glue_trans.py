...
def main():
    ...
    orimodel = model_class.from_pretrained(
    ...
    origin_model_dict = rewind(orimodel.state_dict())
    mask_dict = torch.load(args.mask, map_location='cpu')

    model = torch.load(os.path.join(args.pretrained, "model.pt"), map_location='cpu')
    print('loading model from', args.pretrained)
    zero_rate = see_weight_rate(model)
    print('model 0:',zero_rate)
    
    print('rewind pretrianed bert weight and mask')
