...
def train(args, train_dataset_task1, train_dataset_pre, model, tokenizer):
    """ Train the model """
    record_result1 = []
    record_result_pre = []
    ...
    max_data_length = max(len(train_dataloader_task1), len(train_dataloader_pre))
    ...
    for _ in train_iterator:
        epoch_iterator = tqdm(zip(cycle(train_dataloader_task1),train_dataloader_pre), desc="Iteration", disable=args.local_rank not in [-1, 0])
...
def evaluate(args, task_id, model, tokenizer, prefix=""):
    ...
    task_name_list = [args.task1_name]
    data_dir_list = [args.data_dir1]
    output_mode_list = [args.output_mode1]
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset_task1, train_dataset_pretrain, model, tokenizer)
