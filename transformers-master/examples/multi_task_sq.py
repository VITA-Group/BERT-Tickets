...
def train(args, train_dataset_task1, train_dataset_task2, train_dataset_task3, model, tokenizer):
    """ Train the model """
    record_result1 = []
    record_result2 = []
    record_result3 = []
    ...
    max_data_length = max(len(train_dataloader_task1),len(train_dataloader_task2),len(train_dataloader_task3))
    ...
    for _ in train_iterator:
        epoch_iterator = tqdm(zip(cycle(train_dataloader_task1),cycle(train_dataloader_task2),train_dataloader_task3), desc="Iteration", disable=args.local_rank not in [-1, 0])
        for step, allbatch in enumerate(epoch_iterator):
            ...
            inputs1 = {
                "input_ids": batch1[0],
                "attention_mask": batch1[1],
                "token_type_ids": batch1[2],
                "start_positions": batch1[3],
                "end_positions": batch1[4],
                "task_ids":0,
            }
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results1 = evaluate_squad(args, model, tokenizer)
...
def evaluate_squad(args, model, tokenizer, prefix=""):
    dataset, examples, features = load_and_cache_examples_squad(args, tokenizer, evaluate=True, output_examples=True)
...
def evaluate(args, task_id, model, tokenizer, prefix=""):
    ...
    data_dir_list = [args.data_dir, args.data_dir2, args.data_dir3]
    output_mode_list = [None, args.output_mode2, args.output_mode3]
...
def load_and_cache_examples_squad(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features, dataset = squad_convert_examples_to_features(
            examples=examples,
            tokenizer=tokenizer,
            max_seq_length=args.max_seq_length_sq,
...
def main():
    ...
    if args.doc_stride >= args.max_seq_length_sq - args.max_query_length:
        logger.warning(
    ...
    # Training
    if args.do_train:
        train_dataset_task1 = load_and_cache_examples_squad(args, tokenizer, evaluate=False, output_examples=False)
        ...
        global_step, tr_loss = train(args, train_dataset_task1, train_dataset_task2, train_dataset_task3, model, tokenizer)
